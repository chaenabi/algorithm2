...
# 다시말해 숫자가 높을 수록 중요도가 높습니다.

# 현재 실행 대기 큐(Queue)에 있는 프로세스의 중요도가 순서대로 담긴 배열 priorities와, 
# 몇 번째로 실행되는지 알고싶은 프로세스의 위치를 알려주는 location이 매개변수로 주어질 때, 
# 해당 프로세스가 몇 번째로 실행되는지 return 하도록 solution 함수를 작성해주세요.

def solution(priorities, location): # [2, 1, 3, 2], 2 -> 1
    answer = 0

    # 실행 대기큐에서 대기중인 프로세스 하나를 꺼낸다
    p = priorities.pop()
    for e in priorities:
        if p < e: # 큐에 대기중인 프로세스 중 우선순위가 더 높은 프로세스가 큐에 있다면 
            priorities.append(p) # 꺼낸 프로세스를 다시 큐에 넣는다
